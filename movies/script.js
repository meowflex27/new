...
// === Infinite Scroll by Row ===
let rowObserver = new IntersectionObserver(handleRowIntersection, {
  rootMargin: '100px',
  threshold: 0.5,
});
...
function handleRowIntersection(entries) {
  entries.forEach(entry => {
    if (entry.isIntersecting && canLoadMore) {
      ...
      const url = isGenreMode && currentGenreId
        ? `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=${currentGenreId}&sort_by=popularity.desc&page=${currentPage}`
        : currentType === 'trending'
          ? `https://api.themoviedb.org/3/trending/movie/day?api_key=${apiKey}&page=${currentPage}`
          : `https://api.themoviedb.org/3/movie/${currentType}?api_key=${apiKey}&page=${currentPage}`;
      fetchExploreData(url);
    }
  });
}
...
function loadExploreCategory(type, reset = true) {
  ...
  const url = type === 'trending'
    ? `https://api.themoviedb.org/3/trending/movie/day?api_key=${apiKey}&page=${currentPage}`
    : `https://api.themoviedb.org/3/movie/${type}?api_key=${apiKey}&page=${currentPage}`;
...
function filterExploreByGenre(genreId) {
  ...
  const url = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_genres=${genreId}&sort_by=popularity.desc&page=${currentPage}`;
  exploreGrid.innerHTML = '';
  fetchExploreData(url);
}
...
async function fetchExploreData(url) {
  try {
    ...
    const movieElements = await Promise.all(data.results.map(async (movie) => {
      const id = movie.id;
    ...
    const lazyImages = exploreGrid.querySelectorAll('img.lazy-img:not(.loaded)');
    lazyImages.forEach(img => lazyImageObserver.observe(img));

    const items = exploreGrid.querySelectorAll('.explore-item');
    if (items.length) {
      const lastItem = items[items.length - 1];
      rowObserver.observe(lastItem);
    }
...
// === Auto-update daily based on date ===
...
// === Auto Switch Posters on Resize (Desktop ↔ Mobile) ===
let resizeTimeout;
  ...
  resizeTimeout = setTimeout(() => {
    ...
    if (isMobileNow !== lastIsMobile) {
      ...
      if (isGenreMode && currentGenreId) {
        filterExploreByGenre(currentGenreId);
      } else {
        loadExploreCategory(currentType, true);
      }
    }
